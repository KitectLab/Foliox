...
class LazyPagingItems<T : Any> (
     ...
     * calling [refresh] will also trigger a call to [androidx.paging.RemoteMediator.load] with [androidx.paging.LoadType.REFRESH]
...
fun <T : Any> Flow<PagingData<T>>.collectAsLazyPagingItems(
    ...
    val lazyPagingItems = remember(this) { toLazyPagingItems() }
    lazyPagingItems.collect(context)

    return lazyPagingItems
}

fun<T: Any> Flow<PagingData<T>>.toLazyPagingItems() = LazyPagingItems(this)

@Composable
fun <T : Any> LazyPagingItems<T>.collect(context: CoroutineContext = EmptyCoroutineContext) {
    LaunchedEffect(this) {
 ...
 * because every event from [androidx.paging.PagingDataPresenter] will trigger a recomposition.
 *
 ...
 * [androidx.paging.PagingDataPresenter] will trigger a recomposition.
 *
 ...
 * If [androidx.paging.PagingConfig.enablePlaceholders] is true, LazyPagingItems may return null items. Null
